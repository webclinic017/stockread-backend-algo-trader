...
from src.autotrade import WealthSimpleConnector
from src.autotrade import InvalidSecurityIdError
from src.autotrade.broker.connection.ws_order import Order, OrderType


class WealthSimpleTickerClient:
    def __init__(self, ws_connector: WealthSimpleConnector, ticker_symbol: str, account_id: str):
    ...
    def _find_security(self):
        resp = self.ws_connector.ws_access.find_securities(ticker=self.ticker_symbol)
    ...
    def get_positions(self):
        resp = self.ws_connector.ws_access.get_positions(sec_id=self.sec_id, account_id=account_id)
        positions_dict = dict()
    ...
    def get_buy_orders(self) -> list:
        resp = self.ws_connector.ws_access.get_activities(type='buy', sec_id=self.sec_id, account_id=self.account_id)
        return resp['results']

    def get_sell_orders(self) -> list:
        resp = self.ws_connector.ws_access.get_activities(type='sell', sec_id=self.sec_id, account_id=self.account_id)
        return resp['results']

    def make_limit_buy_order(self, limit_price: float, quantity: int):
        try:
            resp = self.ws_connector.ws_access.limit_buy_order(security_id=self.sec_id,
    ...
    def make_target_buy_order(self, stop_price: float, limit_price: float, quantity: int):
        try:
            resp = self.ws_connector.ws_access.stop_limit_buy_order(security_id=self.sec_id,
    ...
    def make_market_buy_order(self, quantity: int):
        try:
            resp = self.ws_connector.ws_access.market_buy_order(security_id=self.sec_id,
                                                                quantity=quantity,
                                                                account_id=self.account_id)
            order = Order(result=resp)
            self.pending_buy_orders.append(order)
    ...
    def make_limit_sell_order(self, limit_price: float, quantity: int):
        try:
            resp = self.ws_connector.ws_access.limit_sell_order(security_id=self.sec_id,
                                                                limit_price=limit_price,
                                                                quantity=quantity,
                                                                account_id=self.account_id)
            order = Order(result=resp)
    ...
    def make_stop_loss_order(self, stop_price: float, limit_price: float, quantity: int):
        try:
            resp = self.ws_connector.ws_access.stop_limit_sell_order(security_id=self.sec_id,
                                                                     stop_price=stop_price,
                                                                     limit_price=limit_price,
                                                                     quantity=quantity,
                                                                     account_id=self.account_id)
    ...
    def make_market_sell_order(self, quantity: int):
        try:
            resp = self.ws_connector.ws_access.market_sell_order(security_id=self.sec_id,
                                                                 quantity=quantity,
                                                                 account_id=self.account_id)
            order = Order(result=resp)
            self.pending_sell_orders.append(order)
    ...
    def cancel_pending_orders(self, order_type: OrderType):
        ...
        for order in pending_orders:
            self.ws_connector.ws_access.cancel_order(order.order_id)
...
if __name__ == '__main__':
    ws_connector = WealthSimpleConnector()
    ws_connector.connect()
    account_id = "tfsa-hyrnpbqo"
    SHOP_wtc = WealthSimpleTickerClient(ws_connector, 'SHOP', account_id=account_id)
