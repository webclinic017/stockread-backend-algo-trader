...
class RSIMFIBollStrategy2(BaseStrategy):
    ...
    def __init__(self):
        ...
        self.mband_buy_signal = Signal(isbuy=True, codename='MiddleBollBuy', sequence='only')
    ...
    def next(self):
        ...
        if abs(self.bars[0].rsi_uphit_30) == 1:
            ...
            if not self.position.has_position():
                ...
                self.mband_buy_signal.down_signal()
        ...
        if abs(self.bars[0].price_upcross_mband) == 1:
            if not self.position.has_position():
                self.mband_buy_signal.up_signal(ref_bar=self.bars[0],
                                                ref_indicator_value={'bollinger_mavg': self.bars[0].bollinger_mavg})
        ...
        if self.buy_sgnl_set.is_up and self.mband_buy_signal.is_up:
            if not self.position.has_position():
