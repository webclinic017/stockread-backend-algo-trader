...
class RSIMFIStrategy(BaseStrategy):
    ...
    def next(self):
        ...
        if abs(self.bars[0].rsi_uphit_30) == 1:
            self.rsi_buy_signal.set_note('RSI up hits 30')
        ...
        if abs(self.bars[0].mfi_uphit_20) == 1:
            self.mfi_buy_signal.set_note('MFI up hits 20')
            if not self.position.has_position():
        ...
        if abs(self.bars[0].rsi_downhit_70) == 1:
            self.rsi_sell_signal.set_note('RSI down hits 70')
        ...
        if abs(self.bars[0].mfi_downhit_80) == 1:
            self.mfi_sell_signal.set_note('MFI down hits 80')
            if self.position.has_position():
        ...
        if self.sell_sgnl_set.is_up:
            # if self.position.has_position():
            if self.is_bought:
                # self.sell(islimit=False, ref_price=self.bars[0].close)
                # print('MAKE A FUCKING SELL')
