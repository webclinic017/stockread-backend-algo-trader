...
File Name: rest_api/rest_api.py
...
from .errors import TSXStopLimitPriceError
...
class Wsimple:
    """Wsimple is the main access class to the connection rest_api."""
    ...
    def __init__(
        ...
        if self.public_mode:
            ...
            self.session_status_code = r.status_code
            self.logger.debug(f"Pre-login: {r.status_code}/ {str(r.content)}")
            if "x-connection-otp-required" in r.headers:
                self.device_id = r.headers["x-ws-device-id"]

                otp_headers = (
                    r.headers["x-connection-otp"].replace(" ", "").split(";")
                )
            ...
            if self.internally_manage_tokens:
                self.box = TokensBox(
                    ...
                    datetime.fromtimestamp(
                        int(r.headers["X-Access-Token-Expires"])
                    ),
                )
            else:
    ...
    def public(cls, verbose=False):
        """
        constructor: Use Wsimple.public functions without an connection account:
        """
    ...
    @_manage_tokens
    def get_historical_portfolio_data(
            self, tokens=None, time: str = "1d", account_id: Optional[str] = None
    ):
        """
        ...
        params = {}
        if account_id == None:
            params["account_id"] = self.accounts(tokens=tokens).personal
        return requestor(
    ...
    def _send_order(self, payload: dict, tokens=None):
        """ send order to connection servers """
        return requestor(
    ...
    def market_buy_order(
        ...
        """
        try:
            self.logger.debug("buy_market_order")
    ...
    def limit_buy_order(
        ...
        """
        try:
            self.logger.debug("buy_limit_order")
            if account_id is None:
                account_id = self.accounts(tokens=tokens).personal
            return self._send_order(
                {
    ...
    def market_sell_order(
        ...
        """
        try:
            self.logger.debug("sell_market_order")
            if account_id is None:
                account_id = self.accounts(tokens=tokens).personal
            return self._send_order(
                {
    ...
    def limit_sell_order(
        ...
        """
        try:
            self.logger.debug("sell_limit_order")
            if account_id is None:
                account_id = self.accounts(tokens=tokens).personal
            return self._send_order(
                {
    ...
    @_manage_tokens
    def find_securities(
            self, ticker: str, tokens=None, offset=None, limit=None, fuzzy=False
    ):
        """
        ...
        req = requestor(
            ...
            args={"base": self.BASE_URL},
            headers=tokens[0],
            params={"query": ticker, **params},
            logger=self.logger,
        )
        if fuzzy:
            return req
        else:
            return next(filter(lambda x: x.stock.symbol == ticker, req.results), None)

    @_manage_tokens
    def find_securities_by_id(
            self,
            sec_id: str,
            tokens=None,
    ):
    ...
    @_manage_tokens
    def exchange_to(self, value: Union[int, float, str], to: str, tokens=None) -> None:
        rates = self.get_exchange_rate(tokens=tokens)["USD"]
        if to == "USD":
            return float(value) / rates["buy_rate"]
        elif to == "CAD":
            return float(value) * rates["sell_rate"]
    ...
    @_manage_tokens
    def get_top_losers_securities(self, tokens=None, offset: int = 0, limit: int = 20):
        """
    ...
    @_manage_tokens
    def get_top_gainers_securities(self, tokens=None, offset: int = 0, limit: int = 20):
        """
    ...
    @_manage_tokens
    def get_most_active_securities(self, tokens=None, limit: int = 20):
        """
    ...
    @_manage_tokens
    def get_most_watched_securities(
            self, tokens=None, offset: int = 0, limit: int = 20
    ):
        """
    ...
    @_manage_tokens
    def get_all_securities_groups(
            self, tokens=None, offset: int = 0, limit: int = 25, order: str = "desc"
    ):
        """
    ...
    def get_cleaned_monthly_statements(self, tokens=None):
        ...
        try:
            ...
            monthly_statements_data = self.get_monthly_statements(tokens=tokens)[
                "results"
            ]
            result = []
            for data in monthly_statements_data:
                url_response = self.get_monthly_statements_url(
                    data["id"], tokens=tokens
                )
    ...
    def get_websocket_uri(self, tokens=None):
        """
        returns websocket url to a connection websockets
        "wss://trade-utility.connection.com/websocket?ticket={{TICKET}}&version=2"
        """
        res = requestor(
        ...
        return (
            "wss://trade-utility.connection.com/websocket?ticket={}&version=2".format(
                res.ticket
            )
        )
    ...
    def test_endpoint(self, data, tokens=None):
        ...
        #"""
        f = requests.delete(url="{}account".format(self.BASE_URL), headers=tokens[0])
    ...
    @_manage_tokens
    def stock(self, sec_id: str, tokens=None, time: str = "1m"):
        """
        ...
        try:
            ...
            return {
                "sparkline": self.find_securities_by_id_historical(
                    sec_id, time=time, tokens=tokens
                ),
                "security_info": info,
    ...
    def search_page(self, tokens=None):
        ...
        try:
            return {
                "featured_security_groups": self.get_featured_security_groups(
                    tokens=tokens
                ),
    ...
    def dashboard(self, tokens=None):
        ...
        """
        self.logger.debug("calling dashboard")
        try:
            ...
            previous_amount = account_data["previous_close_net_liquidation_value"][
                "amount"
            ]
    ...
    # ! public prefix functions: connection operational status
    ...
    def current_status(self):
        """
        Get current status/incidents of connection trade.
    ...
    def historical_status(self):
        """
        Get all previous history status/incidents of connection trade.
        the data is in body(content), data could be large.
        """
        return requestor(
